...
regmixEM <- function (y, x, lambda = NULL, beta = NULL, sigma = NULL, k = 2,
                      addintercept = TRUE, arbmean = TRUE, arbvar = TRUE, epsilon = 1e-08, maxit = 500, verb = FALSE) {
...
regmixmodel.sel.wy <- function (x, y, k = 2) {
    ...
    for (i in 1: k) {
        if (i == 1) {
            ...
            emout <- regmixEM(y, x, beta = beta, k = i, lambda = c(0.7, 0.3))
        }
...
# args <- c('./sim/sim_x.csv', './sim/output') # test input (comment out after test)
