...
'
iTARGEX significant regulator candidates analysis.

Usage:
  itargex_analysis.R [options] <input-trait> <deletome-data> <output-folder>
  itargex.R -h | --help
...
library(doParallel)
library(scales)
loadNamespace("weights")

del_array <- fread(file = args$deletome_data, nThread = args$ncpus) # deletion array
...
weight_c1 <- as.matrix(fread(file = file.path(output_dir, "weight_comp1.csv"), nThread = args$ncpus))
weight_c2 <- as.matrix(fread(file = file.path(output_dir, "weight_comp2.csv"), nThread = args$ncpus))
...
cor_df <- foreach(i = 1: ncol(y), .combine = 'comb', .multicombine = TRUE, .packages = c("weights")) %dopar% {
# for (i in 1: ncol(y)) {
    ...
    # Soft assign
    cor_comp1 <- wtd.cor(x, y[, i], weight = weight_c1[i, ])["Y", "correlation"]
    cor_comp2 <- wtd.cor(x, y[, i], weight = weight_c2[i, ])["Y", "correlation"]
...
cor_df <- cbind(cor_df, param$lambda_1, param$lambda_2, param$iter)
colnames(cor_df)[17:19] <- c("ratio_1", "ratio_2", "iter")

# Write out the dataframe with all data
fwrite(format(cor_df, digits = 6), file = file.path(output_dir, "correlation.csv"), quote = F, row.names = T, nThread = args$ncpus)
...
#                      select = c(1: 3, 13, 17))
cor_sig_c1 <- subset(cor_df, hard_adjp_c1 > -log10(0.001) & hard_adjp_c1 > hard_adjp_c2 & soft_adjp_c1 > -log10(0.001) & ratio_1 >= args$ratio,
                     select = c(1: 3, 13, 17, 19))
# cor_sig_c2 <- subset(cor_df, hard_adjp_c2 > -log10(0.001) & !(hard_adjp_c1 > -log10(0.01)),
#                      select = c(4: 6, 14, 18))
cor_sig_c2 <- subset(cor_df, hard_adjp_c2 > -log10(0.001) & hard_adjp_c2 > hard_adjp_c1 & soft_adjp_c2 > -log10(0.001) & ratio_2 >= args$ratio,
                     select = c(4: 6, 14, 18, 19))
...
colnames(cor_sig_local) <- c("cor", "beta", "pval_beta", "adjp_beta", "ratio", "iter")
